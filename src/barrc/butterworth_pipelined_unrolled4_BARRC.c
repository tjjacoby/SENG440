/*
 * File: butterworth_pipelined_unrolled4_BARRC.c
 *
 * Purpose: Software-pipelined and four-sample-unrolled fixed-point
 *          Butterworth IIR benchmark.
 *
 * Coding standard: Embedded C Coding Standard (BARR-C:2018), Barr Group.
 *
 * Project deviations:
 * - The requested filename retains uppercase characters and does not contain
 *   "main".
...
#define UNROLLED_SAMPLE_COUNT       (UINT32_C(4))
#define NANOSECONDS_PER_SECOND      (INT64_C(1000000000))

typedef struct
{
    int32_t input_history_recent;
    int32_t output_history_recent;
    int32_t output_history_older;
    int32_t current_input;
    int32_t current_feedforward;
} filter_pipeline_state_t;
...
/*
 * Prepare the next feedforward value and complete the current pipeline value.
 */
static inline void
filter_pipeline_advance (int32_t const next_input,
                         filter_pipeline_state_t * const p_pipeline_state)
{
    int32_t next_feedforward =
        (FILTER_FEEDFORWARD_OUTER * next_input)
        + (FILTER_FEEDFORWARD_CENTER * p_pipeline_state->current_input);
    int32_t const accumulator = p_pipeline_state->current_feedforward
        + (FILTER_FEEDBACK_RECENT
           * p_pipeline_state->output_history_recent);
    int32_t const current_output = filter_round_to_odd (
        accumulator
            + (FILTER_FEEDBACK_OLDER
               * p_pipeline_state->output_history_older));

    next_feedforward += FILTER_FEEDFORWARD_OUTER
                        * p_pipeline_state->input_history_recent;
    p_pipeline_state->input_history_recent =
        p_pipeline_state->current_input;
    p_pipeline_state->current_input = next_input;
    p_pipeline_state->output_history_older =
        p_pipeline_state->output_history_recent;
    p_pipeline_state->output_history_recent = current_output;
    p_pipeline_state->current_feedforward = next_feedforward;
}

/*
 * Complete the final sample that remains in the software pipeline.
 */
static inline int32_t
filter_pipeline_complete (
    filter_pipeline_state_t const * const p_pipeline_state)
{
    int32_t accumulator = p_pipeline_state->current_feedforward
        + (FILTER_FEEDBACK_RECENT
           * p_pipeline_state->output_history_recent);

    accumulator += FILTER_FEEDBACK_OLDER
                   * p_pipeline_state->output_history_older;

    return (filter_round_to_odd (accumulator));
}

/*
 * Run the software pipeline with four samples advanced per loop iteration.
 */
static int32_t
filter_run_iir (int32_t const * const p_input_samples,
                uint32_t const sample_count)
{
    int32_t final_output = 0;

    if (0U < sample_count)
    {
        filter_pipeline_state_t pipeline_state =
        {
            0,
            0,
            0,
            p_input_samples[0U],
            FILTER_FEEDFORWARD_OUTER * p_input_samples[0U]
        };
        uint32_t sample_index = 0U;

        while ((sample_count - sample_index) > UNROLLED_SAMPLE_COUNT)
        {
            filter_pipeline_advance (
                p_input_samples[sample_index + 1U],
                &pipeline_state);
            filter_pipeline_advance (
                p_input_samples[sample_index + 2U],
                &pipeline_state);
            filter_pipeline_advance (
                p_input_samples[sample_index + 3U],
                &pipeline_state);
            filter_pipeline_advance (
                p_input_samples[sample_index + 4U],
                &pipeline_state);
            sample_index += UNROLLED_SAMPLE_COUNT;
        }

        while (sample_index < (sample_count - 1U))
        {
            filter_pipeline_advance (
                p_input_samples[sample_index + 1U],
                &pipeline_state);
        ...
        final_output = filter_pipeline_complete (&pipeline_state);
    }
