/*
 * File: butterworth_pipelined_BARRC.c
 *
 * Purpose: Software-pipelined fixed-point Butterworth IIR benchmark.
 *
...
/*
 * Run the software-pipelined direct-form IIR filter.
 *
 * The next feedforward value is prepared before the current recursive output
 * is completed. The filter's output dependency is therefore preserved.
 */
static int32_t
                ...
                uint32_t const sample_count)
{
    ...
    if (0U < sample_count)
    {
        ...
        int32_t current_input = p_input_samples[0U];
        ...
        for (uint32_t sample_index = 0U;
             ...
             sample_index++)
        {
            int32_t const next_input =
                p_input_samples[sample_index + 1U];
            int32_t next_feedforward =
                (FILTER_FEEDFORWARD_OUTER * next_input)
                + (FILTER_FEEDFORWARD_CENTER * current_input);
            int32_t const accumulator = current_feedforward
                + (FILTER_FEEDBACK_RECENT * output_history_recent);
            int32_t const current_output = filter_round_to_odd (
                accumulator
                    + (FILTER_FEEDBACK_OLDER * output_history_older));

            next_feedforward +=
                FILTER_FEEDFORWARD_OUTER * input_history_recent;
            input_history_recent = current_input;
        ...
        final_output = filter_round_to_odd (
            current_feedforward
                + (FILTER_FEEDBACK_RECENT * output_history_recent)
                + (FILTER_FEEDBACK_OLDER * output_history_older));
    }
