/*
 * File: butterworth_unrolled4_BARRC.c
 *
 * Purpose: Four-sample-unrolled fixed-point Butterworth IIR benchmark.
 *
...
/*
 * Process one input while retaining the recursive IIR state.
 */
static inline int32_t
filter_process_sample (int32_t const current_input,
                       filter_state_t * const p_filter_state)
{
    int32_t const accumulator =
        (FILTER_FEEDFORWARD_OUTER * current_input)
        + (FILTER_FEEDFORWARD_CENTER
           * p_filter_state->input_history_recent);
    int32_t const current_output = filter_round_to_odd (
        accumulator
            + (FILTER_FEEDFORWARD_OUTER
               * p_filter_state->input_history_older)
            + (FILTER_FEEDBACK_RECENT
               * p_filter_state->output_history_recent)
            + (FILTER_FEEDBACK_OLDER
               * p_filter_state->output_history_older));

    p_filter_state->input_history_older =
        p_filter_state->input_history_recent;
    p_filter_state->input_history_recent = current_input;
    p_filter_state->output_history_older =
        p_filter_state->output_history_recent;
    p_filter_state->output_history_recent = current_output;

    return (current_output);
}

/*
 * Run the direct-form IIR filter with four samples per loop iteration.
 */
static int32_t
                ...
                uint32_t const sample_count)
{
    ...
    {
        (void)filter_process_sample (
            p_input_samples[sample_index], &filter_state);
        (void)filter_process_sample (
            p_input_samples[sample_index + 1U], &filter_state);
        (void)filter_process_sample (
            p_input_samples[sample_index + 2U], &filter_state);
        (void)filter_process_sample (
            p_input_samples[sample_index + 3U], &filter_state);
    ...
    return (filter_state.output_history_recent);
}
