...
static int run_IIR(const int *x, int sample_count)
{
    ...
    /*
     * Pipeline prologue: load the first input and calculate its feedforward
    ...
    for (i = 0; i + 1 < sample_count; i++) {
        /*
         * Pipeline stage 1: prepare the next sample. These operations depend
         * only on input history, so they can be scheduled while the current
         * sample is waiting on its feedback dependency.
         */
        register int next_input = x[i + 1];
        register int next_b0_product = b02_1 * next_input;
